...
from ..env.scheduler_simulator import *
...
class CustomTorchModel(nn.Module):
    ...
    @staticmethod
    def list_models():
        return CustomTorchModel.models
...
class CustomActorCriticPolicy(ActorCriticPolicy):
    ...
    def __init__(self, observation_space, action_space,
                 lr_schedule, activation_fn=nn.Tanh,
                 *args, **kwargs):
        """Initialize the policy based on Stable Baselines' ActorCriticPolicy"""

        self.custom_kwargs, kwargs = self.extract_custom_kwargs(**kwargs)
    ...
    def extract_custom_kwargs(self, **kwargs):
        """Extract custom kwargs from kwargs"""
        custom_kwargs = {}
        filtered_kwargs = {}
        for key in kwargs:
            if key in ['actor_model', 'critic_model', 'attn']:
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        actor_model = self.custom_kwargs['actor_model'] if 'actor_model' in self.custom_kwargs else 'kernel'
        critic_model = self.custom_kwargs['critic_model'] if 'critic_model' in self.custom_kwargs else 'critic_lg'
        attn = self.custom_kwargs['attn'] if 'attn' in self.custom_kwargs else False
        self.mlp_extractor = CustomTorchModel(actor_model, critic_model, attn)
    ...
    def ret_actions(self, latent_pi, mask, deterministic=False, ret_dist=False, sample=True):
        ...
        actions = actions + (mask - 1) * 1000000
